...
function hideVideoPopup() {
  ...
  bgMusic.play();
  intervalID = setInterval(game.loop, 1000 / Snake.info.fps);
}
...
const INITIAL_PLAYER = { x: Math.floor(tileCount / 2), y: Math.floor(tileCount / 2) };
  ...
  deathVideo = document.getElementById('deathVideo');
  ...
  var game = {
        ...
        reset: function () {
      // 🐍 CHECK FOR DEATH AND SHOW POPUP 🎥
      var wasMoving = (velocity.x != 0 || velocity.y != 0);
      
      // Stop the interval temporarily to prevent double reset when video is shown
      ...
      if (wasMoving) {
          ...
          // Restart the interval if no video popup was shown (e.g., initial setup or manual ESC reset)
          intervalID = setInterval(game.loop, 1000 / Snake.info.fps);
      }
    },
    ...
    loop: function () {
      ...
      ctx.fillText("topscore: " + pointsMax, 288, 60);
